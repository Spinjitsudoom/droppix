...
distrobox enter "${NAME}" -- bash -lc '
  ...
  sudo dnf install -y gcc-c++ cmake git libevdi kscreen ffmpeg-devel x264-devel 2>/dev/null || \
  sudo dnf install -y gcc-c++ cmake git libevdi ffmpeg-devel x264-devel
'
