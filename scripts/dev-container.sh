...
distrobox enter "${NAME}" -- bash -lc '
  ...
  sudo dnf install -y gcc-c++ cmake git libevdi kscreen ffmpeg ffmpeg-devel x264-devel qt6-qtbase-devel 2>/dev/null || \
  sudo dnf install -y gcc-c++ cmake git libevdi ffmpeg ffmpeg-devel x264-devel qt6-qtbase-devel
'
